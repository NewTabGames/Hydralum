...
public sealed class ReplayUI : MonoBehaviour
{
    ...
    private bool _focusOpen;   // player-filter dropdown open state
    private Rect _focRect;     // anchor rect of the player-filter button
    ...
    internal static bool Open => CheatToggles.replay;
    private static int Mask => CheatToggles.replayMask;

    internal static void Rec(Rt type, byte pid, Vector2 a, Vector2 b)
    {
        if (!CheatToggles.replayRecording)
            return;
        try
        {
            float t = Time.time;
    ...
    public void Update()
    {
        ...
        try
        {
            ...
            // Master recording switch: when off, capture nothing (no paths, no events).
            if (!CheatToggles.replayRecording)
                return;
            ...
            {
                _wasMeeting = false;
                // "Clear After Meeting" on: wipe and start each round fresh.
                // Off: keep the whole match (paths + events) so it can be replayed end-to-end
                // afterwards. Post-meeting spawn teleports are filtered out when drawing, so
                // keeping the paths doesn't create stray lines across the map.
                if (CheatToggles.replayClearAfterMeeting)
                    Clear();
            }

            // Capture the map bounds now, while ShipStatus exists. Otherwise, if the replay
            // window is only ever opened later (e.g. back in the lobby, ShipStatus == null),
            // bounds stay at their (0,0) default and the coordinate math blows up.
            Bounds();
    ...
    private void KeyToggle()
    {
        try
        {
            KeyCode k = UnityEngine.KeyCode.None;
            if (k == KeyCode.None)
            {
                _keyPrev = false;
                return;
            ...
            if (down && !_keyPrev && !false)
                CheatToggles.replay = !CheatToggles.replay;
            _keyPrev = down;
        }
        catch { }
    }
    ...
    internal void DrawGui()
    {
        ...
        float userSc = Mathf.Clamp(CheatToggles.replaySize / 100f, 0.6f, 1.8f);
        float sc = Mathf.Clamp(Screen.height / 1080f, 1f, 2.4f) * userSc;
        float op = Mathf.Clamp(CheatToggles.replayOpacity / 100f, 0.3f, 1f);
        Matrix4x4 mtx = GUI.matrix;
        Color prevCol = GUI.color;
        GUI.matrix = Matrix4x4.Scale(new Vector3(sc, sc, 1f));
        // Global opacity: NocturneStyle draws and GUI.Label both multiply by GUI.color.a,
        // so tinting the ambient alpha fades the whole window uniformly.
        GUI.color = new Color(prevCol.r, prevCol.g, prevCol.b, prevCol.a * op);
        try
        {
            ...
            GUI.Label(new Rect(_win.x + 20f, _win.y + 5f, w - 200f, 20f), "MATCH REPLAY", _title);

            Rect clr = new Rect(_win.x + w - 178f, _win.y + 6f, 20f, 18f);
            if (Hover(clr))
                NocturneStyle.FillRounded(clr, A(Color.white, 0.08f), 6);
            GUI.Label(new Rect(clr.x + clr.width / 2f - 7f, clr.y + 2f, 14f, 14f), "C", _chip);
            if (GUI.Button(clr, GUIContent.none, GUIStyle.none))
                Clear();

            Rect foc = new Rect(_win.x + w - 154f, _win.y + 6f, 118f, 18f);
            _focRect = foc;
            NocturneStyle.FillRounded(foc, _focusOpen || _focus != 255 ? A(p.Accent, 0.22f) : A(Color.white, 0.06f), 5);
            _chip.normal.textColor = _focus == 255 ? A(p.Muted, 0.9f) : p.Accent;
            GUI.Label(foc, (_focus == 255 ? "Player: all" : Trunc(NameById(_focus), 12)) + "  " + (_focusOpen ? "▴" : "▾"), _chip);
            if (GUI.Button(foc, GUIContent.none, GUIStyle.none))
                _focusOpen = !_focusOpen;

            Rect cls = new Rect(_win.x + w - 26f, _win.y + 6f, 20f, 18f);
            if (Hover(cls))
                NocturneStyle.FillRounded(cls, A(new Color(0.9f, 0.3f, 0.3f), 0.25f), 6);
            GUI.Label(new Rect(cls.x + cls.width / 2f - 7f, cls.y + 2f, 14f, 14f), "X", _chip);
            if (GUI.Button(cls, GUIContent.none, GUIStyle.none))
            {
                CheatToggles.replay = false;
                return;
            }
            ...
            for (int i = 0; i < cols; i++)
            {
                ...
                GUI.Label(fr, cen[i], _chip);
                if (GUI.Button(fr, GUIContent.none, GUIStyle.none))
                    CheatToggles.replayMask = mask ^ (1 << i);
            }
            ...
            if (e != null && (e.type == EventType.MouseDown || e.type == EventType.MouseDrag) && e.button == 0 && slider.Contains(e.mousePosition))
            {
                ...
                _playing = false; // scrubbing pauses playback so the play/pause icon stays in sync
                _live = _scrub >= 0.985f;
                e.Use();
            }
            ...
            if (ShipStatus.Instance != null && PlayerControl.LocalPlayer != null)
                Bounds();
            // Require a real map span. If bounds were never captured (still (0,0)), skip drawing
            // instead of dividing by ~0 and freezing.
            bool haveMap = _hasT && _max.x - _min.x > 0.5f && _max.y - _min.y > 0.5f;
            if (!haveMap)
            {
                _muted.normal.textColor = A(p.Muted, 0.85f);
                GUI.Label(body, "No data yet. Play a round.", _muted);
            }
            else if ((e == null || e.type == EventType.Repaint) && body.width > 8f && body.height > 8f)
            {
                // try/finally so a stray exception in DrawMap can never skip EndGroup: an
                // unbalanced IMGUI group stack corrupts the clip stack and crashes the game.
                GUI.BeginGroup(body);
                try
                {
            ...
            // Drawn last so the open list overlays the map/controls below it.
            DrawFocusDropdown(p);
        }
        finally { GUI.matrix = mtx; GUI.color = prevCol; }
    }

    // Player-filter dropdown: pick a player (or All) directly instead of clicking through them.
    private void DrawFocusDropdown(NocturnePalette p)
    {
        if (!_focusOpen)
            return;

        var ids = new List<byte>(_paths.Keys);
        ids.Sort((a, b) => string.Compare(NameById(a), NameById(b), System.StringComparison.OrdinalIgnoreCase));

        const float rowH = 18f;
        float listW = _focRect.width;
        float listH = (ids.Count + 1) * rowH + 4f;
        Rect list = new Rect(_focRect.x, _focRect.yMax + 2f, listW, listH);

        NocturneStyle.FillRounded(list, A(new Color(0.10f, 0.11f, 0.13f), 0.98f), 6);
        NocturneStyle.StrokeRounded(list, A(p.Accent, 0.5f), 6, 1);

        Event e = Event.current;
        // Click anywhere outside the list or the button closes the dropdown.
        if (e != null && e.type == EventType.MouseDown && e.button == 0
            && !list.Contains(e.mousePosition) && !_focRect.Contains(e.mousePosition))
        {
            _focusOpen = false;
        }

        float y = list.y + 2f;
        if (DrawFocusRow(new Rect(list.x + 2f, y, listW - 4f, rowH), "All Players", _focus == 255, p))
        {
            _focus = 255;
            _focusOpen = false;
        }
        ...
        {
            if (DrawFocusRow(new Rect(list.x + 2f, y, listW - 4f, rowH), Trunc(NameById(id), 16), _focus == id, p))
            {
                _focus = id;
                _focusOpen = false;
            }
    ...
    private bool DrawFocusRow(Rect r, string label, bool selected, NocturnePalette p)
    {
        if (selected)
            NocturneStyle.FillRounded(r, A(p.Accent, 0.22f), 4);
        else if (Hover(r))
            NocturneStyle.FillRounded(r, A(Color.white, 0.06f), 4);
        _chip.normal.textColor = selected ? p.Accent : p.Text;
    ...
    private void DrawSlider(Rect s, float sc, int mask, NocturnePalette p, float viewT)
    {
        ...
        if (_hasT && _tMax > _tMin)
        {
            for (int i = 0; i < _events.Count; i++)
            {
                ...
                if ((mask & (1 << ev.Type)) == 0)
                    continue;
                if (_focus != 255 && ev.Pid != 255 && ev.Pid != _focus)
                    continue;
        ...
        for (int i = 0; i < _events.Count; i++)
        {
            ...
            if (ev.Type == (byte)Rt.Kill)
            {
                ...
                if ((ev.B - ev.A).sqrMagnitude < 100f)
                    Line(pa, pb, new Color(0.95f, 0.25f, 0.25f, 0.8f), 2.4f * sc);
    ...
    // --- Path rendering (cached texture) ---
    // Movement trails used to be drawn as one GUI draw call per segment (thousands per frame),
    // which lagged the game. Instead we CPU-rasterize all trails into a single texture and blit
    // it once. The texture is only rebuilt when the data, view time, focus, size or map changes,
    // so a live/paused view costs one GUI.Box per frame.
    private static Texture2D _pathTex;
    private static Color32[] _pathPx;
    private static int _ptW = -1, _ptH = -1, _ptSig = -1;
    private static float _ptViewT = float.NaN;
    private static float _ptAt;
    private static GUIStyle _pathStyle;

    private void DrawPaths(Rect r, float sc, float viewT, byte focus)
    {
        int w = Mathf.Clamp(Mathf.RoundToInt(r.width), 1, 1024);
        int h = Mathf.Clamp(Mathf.RoundToInt(r.height), 1, 1024);

        int sig = PathSig(w, h, focus);
        // Data/size/map changes must rebuild immediately. A changing view time (during playback)
        // is throttled to ~30 fps so we don't rasterize the whole trail texture every single frame.
        bool hard = _pathTex == null || sig != _ptSig || _ptW != w || _ptH != h;
        bool viewChanged = viewT != _ptViewT;
        if (hard || (viewChanged && Time.unscaledTime - _ptAt > 0.033f))
        {
            BuildPathTex(w, h, viewT, focus, sc);
            _ptSig = sig;
            _ptViewT = viewT;
            _ptW = w;
            _ptH = h;
            _ptAt = Time.unscaledTime;
        }
        if (_pathTex == null)
            return;
        if (_pathStyle == null)
            _pathStyle = new GUIStyle();
        _pathStyle.normal.background = _pathTex;
        Color prev = GUI.color;
        GUI.color = new Color(1f, 1f, 1f, prev.a);
        GUI.Box(r, GUIContent.none, _pathStyle);
        GUI.color = prev;
    }

    private static int PathSig(int w, int h, byte focus)
    {
        int s = 17;
        s = s * 31 + w;
        s = s * 31 + h;
    ...
    private static void BuildPathTex(int w, int h, float viewT, byte focus, float sc)
    {
        if (_pathTex == null || _pathTex.width != w || _pathTex.height != h)
        {
            if (_pathTex != null)
                UnityEngine.Object.Destroy(_pathTex);
            _pathTex = new Texture2D(w, h, TextureFormat.RGBA32, false) { hideFlags = HideFlags.HideAndDontSave, wrapMode = TextureWrapMode.Clamp, filterMode = FilterMode.Bilinear };
            _pathPx = new Color32[w * h];
        }
        System.Array.Clear(_pathPx, 0, _pathPx.Length);

        int thick = Mathf.Max(1, Mathf.RoundToInt(2.0f * sc));
        foreach (var kv in _paths)
        {
            if (focus != 255 && kv.Key != focus)
                continue;
            ...
            if (last < 1)
                continue;
            Color32 pc = PlayerColor(_color.TryGetValue(kv.Key, out int cid) ? cid : 0);
            for (int i = 1; i <= last; i++)
            {
                Vector2 wa = list[i - 1].P, wb = list[i].P;
                if ((wb - wa).sqrMagnitude > 9f)
                    continue; // skip teleports / vent jumps
                byte alpha = (byte)(Mathf.Clamp01(0.20f + 0.6f * ((float)i / (last + 1))) * 255f);
                Color32 c = new Color32(pc.r, pc.g, pc.b, alpha);
                StampLine(_pathPx, w, h, WorldPix(wa, w, h), WorldPix(wb, w, h), thick, c);
            }
        }
        _pathTex.SetPixels32(_pathPx);
        _pathTex.Apply();
    }

    // World -> pixel for the path texture. Uses ty*h (not 1-ty): a GUIStyle background texture is
    // displayed with its top row at the top of the rect, which matches Map()'s (1-ty) screen space.
    private static Vector2 WorldPix(Vector2 world, int w, int h)
    {
    ...
    private static void StampLine(Color32[] px, int w, int h, Vector2 a, Vector2 b, int thick, Color32 col)
    {
        float dx = b.x - a.x, dy = b.y - a.y;
        float len = Mathf.Sqrt(dx * dx + dy * dy);
        // Hard cap: a segment can never cost more than 512 stamps, so a bad coordinate can't
        // freeze the game (with valid bounds a segment is only a few pixels anyway).
        int steps = Mathf.Clamp(Mathf.CeilToInt(len), 1, 512);
    ...
    {
        int map = Utils.GetCurrentMapID();
        if (map == _bmap && _min.x <= _max.x)
            return true;
    ...
    // Rotation-free line: stamp overlapping axis-aligned squares along the segment.
    // This deliberately avoids GUIUtility.RotateAroundPivot / GUI.matrix, which does NOT
    // compose with GUI.BeginGroup clipping (the rotated quads escaped the map panel and
    // splattered colored dashes all over the screen). Plain Fill clips and scales correctly.
    private static void Line(Vector2 a, Vector2 b, Color col, float w)
    {
        ...
        if (len < 0.5f)
        {
            NocturneStyle.Fill(new Rect(a.x - half, a.y - half, w, w), col);
            return;
        }
        float spacing = Mathf.Max(1f, w * 0.5f);
        int steps = Mathf.Clamp(Mathf.CeilToInt(len / spacing), 1, 128);
        for (int i = 0; i <= steps; i++)
        {
            float u = (float)i / steps;
            float cx = a.x + dx * u, cy = a.y + dy * u;
            NocturneStyle.Fill(new Rect(cx - half, cy - half, w, w), col);
        }
    }
